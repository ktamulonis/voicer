...
def use_kernel_forward_from_hub(*args, **kwargs):
    # HF optional optimization hook – safe no-op decorator for inference
    def _decorator(obj):
        return obj
...
def create_causal_mask(
    ...
    **kwargs,
    ...
    """
    Minimal causal mask compatible with HF decoder expectations.
    Accepts both legacy and newer Transformers call signatures.
    """
    input_embeds = kwargs.get("input_embeds", None)
    if input_shape is None:
        if input_embeds is None:
            raise ValueError("create_causal_mask requires input_shape or input_embeds.")
        input_shape = input_embeds.shape[:2]
    if device is None:
        device = input_embeds.device if input_embeds is not None else "cpu"
    if dtype is None:
        dtype = input_embeds.dtype if input_embeds is not None else torch.float32
    ...
    mask = torch.zeros((tgt_len, tgt_len + past_len), device=device, dtype=dtype)
    causal_block = torch.full((tgt_len, tgt_len), min_val, device=device, dtype=dtype)
    causal_block = torch.triu(causal_block, diagonal=1)
    mask[:, past_len:] = causal_block
    mask = mask.unsqueeze(0).unsqueeze(1).expand(bsz, 1, tgt_len, tgt_len + past_len)

    if attention_mask is not None:
        # attention_mask is expected to be (bsz, src_len)
        extended = attention_mask[:, None, None, :].to(dtype=dtype)
        extended = (1.0 - extended) * min_val
        mask = mask + extended
...
def create_extended_attention_mask_for_decoder(
    ...
    """
    Minimal extended attention mask.
    """
    if attention_mask is None:
        return None

    extended = attention_mask[:, None, None, :]
    extended = extended.to(dtype=dtype or torch.float32)
...
def create_sliding_window_causal_mask(
    ...
    **kwargs,
    ...
    # Qwen3-TTS doesn't need true sliding-window masking for local inference.
    # Fall back to standard causal mask.
...
except Exception:  # pragma: no cover - compat shim for newer transformers
    class GradientCheckpointingLayer(torch.nn.Module):
        """Fallback base class when HF removes modeling_layers.

        This model only relies on nn.Module behavior for local inference.
        """
...
except Exception:  # pragma: no cover - compat shim for removed helper
    from transformers.modeling_rope_utils import ROPE_INIT_FUNCTIONS

    def dynamic_rope_update(fn):
        """No-op decorator when dynamic RoPE updates are unavailable."""

        return fn
try:
    from transformers.modeling_utils import ALL_ATTENTION_FUNCTIONS, PreTrainedModel
except Exception:  # pragma: no cover - compat shim for removed symbol
...
try:
    ...
    def can_return_tuple(fn):
        """No-op decorator when can_return_tuple is unavailable."""
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Qwen3TTSConfig, **kwargs):
        # Ignore extra kwargs (e.g., device) passed by newer Transformers.
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Work around safetensors metadata returning None in some envs.
        import transformers.modeling_utils as _mu

        _orig_safe_open = getattr(_mu, "safe_open", None)

        def _safe_open_with_metadata(*args, **kwargs):
            f = _orig_safe_open(*args, **kwargs)
            ...
            class _SafeOpenWrapper:
                ...
                def metadata(self):
                    ...
                    return {"format": "pt"} if md is None else md

                def __getattr__(self, name):
                    return getattr(self._inner, name)

            return _SafeOpenWrapper(f)

        if _orig_safe_open is not None:
            _mu.safe_open = _safe_open_with_metadata
        try:
