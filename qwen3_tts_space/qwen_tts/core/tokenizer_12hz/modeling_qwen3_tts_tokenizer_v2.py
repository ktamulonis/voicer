...
def use_kernel_forward_from_hub(*args, **kwargs):
    """
    Compat stub for older Transformers builds.
    In upstream, this returns a decorator that patches kernels.
    Here we just return an identity decorator.
    """
    # Supports both @use_kernel_forward_from_hub and @use_kernel_forward_from_hub(...)
...
def create_causal_mask(
    ...
    **kwargs,
    ...
    """
    Minimal causal mask implementation compatible with HF expectations.
...
def create_extended_attention_mask_for_decoder(
    ...
    """
    Minimal decoder attention mask.
    """
...
def create_sliding_window_causal_mask(
    ...
    **kwargs,
    ...
    # Sliding window is unused by Qwen3-TTS inference → fall back to causal mask
...
try:
    ...
    try:
        ...
        # Inference-only fallback (safe stub)
        class GradientCheckpointingLayer(nn.Module):
            """Fallback base class for inference-only."""
...
try:
    ...
    # Inference-safe no-op fallback
    def dynamic_rope_update(fn):
        return fn
from transformers import PreTrainedModel

try:
    # Never released publicly, but referenced in Qwen code
    from transformers.models.attention import ALL_ATTENTION_FUNCTIONS
except Exception:
    # Inference-safe fallback: attention dispatch not needed for TTS
...
class Qwen3TTSTokenizerV2Model(Qwen3TTSTokenizerV2PreTrainedModel):
    def __init__(self, config: Qwen3TTSTokenizerV2Config, **kwargs):
